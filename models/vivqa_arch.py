...
class ViVQAMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        This function replaces <image> tokens in the text sequence with
        projected image embeddings, and realigns labels, attention masks,
        and position ids accordingly.

        Final output feeds `inputs_embeds` (instead of input_ids) to the LLM.
        """
        vision_tower = self.get_vision_tower()
        image_token_id = getattr(self.config, "image_token_id", None)

        if vision_tower is None or images is None or image_token_id is None or input_ids.shape[1] == 1:
            if (
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_images = (cur_input_ids == image_token_id).sum()
            ...
            image_token_indices = [-1] + torch.where(cur_input_ids == image_token_id)[0].tolist() + [
